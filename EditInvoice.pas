...
begin
  ...
  TWeekReport.Sort := 'Weekbegin DESC';
  TWeekReport.First;

  for I := 0 to TWeekReport.RecordCount -1 do begin
    Index := cmbWeeknr.Items.AddObject(IntToStr(YearOf(TWeekReport.FieldByName('Weekbegin').AsDateTime)) +' ' + TWeekReport.FieldByName('Weeknr').AsString, Pointer(TWeekReport.FieldByName('Id').AsInteger));
