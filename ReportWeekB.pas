...
type
  ...
  private
    ...
    TQuery: TADOQuery;
    Totmoney: Double;
    procedure SetFonts(FontDt: TFontStyles);
  public
...
//  TInvoice.Locate('ID', TWeekReport.FieldByName('FactuurID').AsInteger, []);
  ...
  TQuery.SQL.Text := 'SELECT Werknemers.Voornaam as VoorNaam, Werknemers.Achternaam as Achternaam, Werknemers.UurLoon as UurLoon, Werknemers.IsBedrijf as IsBedrijf, Sum(WeekDetails.Maandag) AS Maandag,'+
...
' GROUP BY Werknemers.Voornaam, Werknemers.Achternaam, Werknemers.UurLoon, Werknemers.IsBedrijf, Weekrapporten.Id;';
  TQuery.ExecSQL;
  TQuery.Open;
  ...
  if aFieldName = 'WerknemerID' then
    ...
    if aFieldName = 'TotaalSalaris' then begin
      ...
      if TQuery.FieldByName(aFieldName+'VZ').AsInteger > 0 then begin
        ...
        if TQuery.FieldByName(aFieldName+'VZ').AsInteger = 5 then
          Result := 'S ' + TQuery.FieldByName(aFieldName).AsString;
      end
      else
...
procedure TfrmReportWeekB.ppDBText31GetText(Sender: TObject;
  var Text: String);
begin
...
procedure TfrmReportWeekB.SetFonts(FontDt: TFontStyles);
begin
    ppDBText1.Font.Style := FontDt;
    ppDBText3.Font.Style := FontDt;
    ppDBText13.Font.Style := FontDt;
    ppDBText6.Font.Style := FontDt;
    ppDBText7.Font.Style := FontDt;
    ppDBText8.Font.Style := FontDt;
    ppDBText9.Font.Style := FontDt;
    ppDBText10.Font.Style := FontDt;
    ppDBText11.Font.Style := FontDt;
end;

procedure TfrmReportWeekB.ppDBText6GetText(Sender: TObject;
  var Text: String);
begin
  if AnsiStartsStr('F ', Text) or AnsiStartsStr('V ', Text) or
  AnsiStartsStr('Z ', Text) or AnsiStartsStr('A ', Text) or
  AnsiStartsStr('S ', Text) then begin
